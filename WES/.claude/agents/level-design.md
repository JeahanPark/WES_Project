...
9. **수치화·공식 일관성** — 밸런스 데이터는 정성 표현("적당히 강한", "약간 어려운") 금지, **실측 가능한 수치**로 박는다. 적 스탯·드롭률·자원밀도는 director의 **난이도 공식**(`D = TP/CAP`)을 기준으로 일관 도출한다 — 지역 깊이 `d`에서 `EnemyScale(d)` 등으로 파생시켜 임의값 난립을 막는다. 임의 수치를 흩뿌리지 말고, 한 base 값 × 공식으로 전 지역을 일관되게.
...
## 7. QA 검증 기준 (나중 효과측정의 합격 기준)
   - 각 수치에 **측정법 + 허용오차** 명시 (예: 드롭률 100회 표본 ±10%p / 체류시간 ±20% / 체감 D ±0.15)
   - 게이트 자원·동작의 합격 조건 (예: "모피 없이 설원 진입 → 동사 발생")
   - 정적 일치(런타임 스탯 == 명세값) / 확률(표본) / 곡선(체류·난이도)으로 분류
   - **이 표가 곧 QA(모드 C) 체크리스트가 되도록** 작성 — 명세값 바꾸면 합격 기준도 갱신
```

> **수치 명세는 곧 테스트 명세다.** 측정 불가능한 수치는 명세가 아니다 — 모든 수치에 "어떻게 측정해 합격을 판정하는가"를 붙인다.
