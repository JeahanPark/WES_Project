...
| **규모** | C# 약 106개 스크립트 / Manager·Controller·Worker·Component 아키텍처 |
...
- **데이터 주도**: 아이템·제작·건물·몬스터·드롭테이블·지역을 모두 CSV로 관리(`Assets/CSVInfo/`), 커스텀 에디터로 ScriptableObject 변환
...
게임 개발 역할을 **4개의 전문 에이전트**로 나누고, 표준 프로세스(부트스트랩 → 기획 합의 → 슬라이스 구현 → 통합 QA → 마무리)에 따라 협업하도록 구성했습니다.
...
└─ .claude/agents/       # 4개 개발 에이전트 정의 (director/client/designer/qa)
```
