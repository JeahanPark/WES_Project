...
## 5. 출시 점검 (2026-06-08, director)

- 구현 품질 양호 — 출시 큰 개편 불필요.
- **슬롯 압박 = 트레이드오프 요소**: 출시 자원/도구/장비 증가 시, 유한 슬롯(초기 20칸)이 **"비축 vs 소모"**(CORE) 결정을 만드는 압력이 됨. 슬롯 무한 확장보다 **유한 슬롯이 결정을 강제**하는 쪽이 코어 재미에 부합 → 슬롯 수·확장 비용은 level-design 튜닝 대상.
- 도면은 슬롯 미점유(도면해금 §5.2 확정) — 인벤토리 압박 없음.
- **변경 의뢰: 없음**(현 구조 유지). 슬롯 수 밸런스만 출시 튜닝.
